...
var
  MenuForm: TMenuForm;
  n_kurs: Integer;
  On_About: Boolean; //положение курсора относительно раздела About
...
begin
  if Key = 38 then                           //up
    Begin
      ...
      //если курсор находится не на разделе About
        Begin
          if n_kurs = 0 then
            Begin
  ...
  {при переносе курсора обратно на раздел PLAY нужно обнулить
   счетчик n_kurs, чтобы курсор не сбился.}
    Begin
  ...
  if Key = 13 then       //enter
    Begin
      if On_About = True then   // если курсор расположен на разделе About
        Begin
          MenuForm.Hide;
          AboutForm.Show;
        End
