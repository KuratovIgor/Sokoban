...
{При нажатии на иконку автопроходжения уровня персонаж сам расставит ящики
 за минимальное количество ходов. Эта функция пока не работает.}
begin
...
{По этому таймеру двигается персонаж, с его помощи обеспечивается плавный анимированный
 переход между участками поля. Для перемещения на 1 клетку достаточно одного нажатия.}
begin
      ...
      if (Location[j_pers,i_pers-1] = 0) or (Location[j_pers,i_pers-1] = 6) or ((Location[j_pers,i_pers-1] = 3) and not(Location[j_pers,i_pers-2] = 1)) then
        Begin
          if LookLeft = True then
            ...
            End;
          if a = 0 then
          {Этот участок кода в таймере выполняется только 1 раз при работе таймера
           для перестановки ящика в массиве.}
            Begin
...
begin
  ...
  Pole6.SendToBack;            //////////////////////////////////////
  Pole1.SendToBack;           //                                  //
  Pole2.SendToBack;          // Выносим все свободные участки    //
  Pole3.SendToBack;         // поля на задний план, чтобы персо-//
  Pole4.SendToBack;        // наж не прятался под ними         //
  Pole5.SendToBack;       //////////////////////////////////////
  Pole7.SendToBack;
  Seconds := 30;
  count_stars := 3;
...
begin
  if Key = 27 then
    ...
    End;
  if Key = 37 then              //left
  {При помощи этих логических переменных определяется, куда мы направили персонажа и
   в какую сторону он смотрит. Все эти переменные используются таймером delay.}
    Begin
      if Already_Press = False then
        Begin
          a := 0;
  ...
  {При нажатии на клавишу "1" активируется телепорт, и персонаж переносится на участок
   поля, соответствующий своим номером номеру нажатой цифры. Всего таких точек на уровнях
   в дальнейшем будет несколько. Для проверки этой функции можно переставить персонажа
   в массие Location на любой участок, персонаж найдет путь к клетке для переноса.}
    Begin
...
begin
  ...
  if Step = 4 then
    Begin                                  ////////////////////////////////////////
      Dec(count_stars);                   //В зависимости от количества ходов   //
      Star3.Visible := False;            //определяется количество полученных  //
    End;                                //звёзд                               //
  if Step = 5 then                     //                                    //
    Begin                             //                                    //
      Dec(count_stars);              //                                    //
...
{Здесь определяется количество использований телепорта.
 Телепортом можно пользоваться только 1 раз за уровень.
 Если эта функция уже использована, ее напдпись будет выделена красным}
begin
  if Teleport_using = True then  //если телепорт уже использован
    Teleport_Time.Enabled := True
  else
    Begin    //старт таймера для телепортации описан в процедуре нажатия клавиш
      Teleport_On := True;
...
{При помощи этого таймера происходит телепортация. В зависимости от выбранного участка
 персонаж ищет нужный маршрут до выбранной точки. Если телепортация исползована, то
 будет выведено соответсвующее сообщение. Переставлять место расположения
 точки для телепортации не рекомендуется :) }
begin
  ...
  {Поиск точки назначения осуществляется при помощи переменных dx_steps и dy_steps.}
    Begin
      if dx_steps < 5 then
        Begin
          dx := dx + 1;
